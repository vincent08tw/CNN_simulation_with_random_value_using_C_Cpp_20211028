...
//			cout << "Matrix_3D_class ID: " << ID << " is created \n";
		...
		void write_content_to_txt_orod(string filename); //one row one data 2021-10-28
		...
		void matrix_convolution(Matrix_3D_class, Matrix_3D_class, int, int);
...
void Matrix_3D_class::matrix_convolution(Matrix_3D_class input, Matrix_3D_class kernel, int stride=1, int Zpadding=0)
{
...
//	int output_matrix_size = ROW_input - ROW_kernel + 1;
	...
	Matrix_3D_class temp_input;
	int temp_size = ROW_input + 2*Zpadding;
	temp_input.set_matrix_size_3D(CNL_input ,temp_size, temp_size);
	
	for(k = 0; k < CNL_input;k++)
	{
		for(i = 0;i < temp_size;i++)
		{
			for(j = 0;j < temp_size;j++)
			{
				...
				{
					temp_input.matrix_content[k][i][j] = 0;
				}
				...
				{
					temp_input.matrix_content[k][i][j] = input.matrix_content[k][i-Zpadding][j-Zpadding];
				}
			}
		}	
	}
			...
			for(b = 0;b < output_matrix_size;b++)
			{
				for(k = 0;k < CNL_kernel;k++)
				{
					for(i = 0;i < ROW_kernel;i++)
					{
						...
						{
							input_slice.matrix_content[k][i][j] = temp_input.matrix_content[k][i+row_bias][j+col_bias];
						}
					}	
				}
				#if DEBUG_MODE == 1
...
/*20211028*/
void Matrix_3D_class::write_content_to_txt_orod(string filename) //one row one value 
{
	size_t channel;
	size_t row;
	size_t column;
		...
		for(channel = 0;channel < this->numberOfChannel;channel++)
		{
		    ...
		    {   
		        ...
		        {   
		        	...
		        	fout.fill(' ');
		        	fout.width(5);
		        	fout << this->matrix_content[channel][row][column] << endl;
		        }
		    }
