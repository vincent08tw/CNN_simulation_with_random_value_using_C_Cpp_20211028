...
v14: LongestPathMatrix Zero_matrix
****************************/
...
//			cout << "Matrix_class ID: " << ID << " is created \n";
		...
		void Zero_matrix(int row, int col); //creat zero matrix
		void LongestPathMatrix(); //Longest Path Matrix Algorithm
...
2021-10-14
*/
void Matrix_class::Zero_matrix(int row,int col)
{
	this->set_matrix_size_2D(row,col);
	int i,j;
	for(i=0;i<row;i++)
	{
		for(j=0;j<col;j++)
...
for VLSIDSP homework1
2021-10-14
*/
void Matrix_class::LongestPathMatrix()
{
	square_matrix_detection(this->get_numberOfRow(),this->get_numberOfColumn());
	int iterationL = 1;
	int D = this->get_numberOfColumn();
	int i,j,k;
	int temp;
	float iterationBound = -1;
	int up,down;
	Matrix_class output;
	Matrix_class tempMatrix;
	Matrix_class zeroMatrix;
	zeroMatrix.Zero_matrix(D,D);
	output.set_matrix_size_2D(D,D);
	tempMatrix.set_matrix_size_2D(D,D);
	
	printf("L%d:\n",iterationL);
	this->print_content();
	
	tempMatrix.matrix_content = this->matrix_content;
	for(iterationL=2;iterationL<=D;iterationL++)
	{
		for(i=1;i<=D;i++)
		{
			for(j=1;j<=D;j++)
			{
				...
				for(k=1;k<=D;k++)
				{
//					printf("test for i=%d, j=%d, k=%d, matrix_content[%d][%d]=%d, tempMatrix.matrix_content[%d][%d]=%d\n",
//							i,j,k,i,k,this->matrix_content[i-1][k-1],k,j,tempMatrix.matrix_content[k-1][j-1]);
					if( (this->matrix_content[i-1][k-1] != -1) && (tempMatrix.matrix_content[k-1][j-1] != -1) )
					{
						if(this->matrix_content[i-1][k-1] + tempMatrix.matrix_content[k-1][j-1] > temp)
						{
							temp = this->matrix_content[i-1][k-1] + tempMatrix.matrix_content[k-1][j-1];
						}
					}
				}
				output.matrix_content[i-1][j-1] = temp;
				if(i==j)
				{
					if(iterationBound*iterationL < temp)
					{
						iterationBound = (float)temp / (float)iterationL;
						up = temp;
						down = iterationL;
					}
				}
		...
		tempMatrix = output + zeroMatrix;
		printf("L%d:\n",iterationL);
		output.print_content();
		
	}
	printf("iteration Bound = %f(%d/%d)\n",iterationBound,up,down);
}
